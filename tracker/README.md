...
- **Weekly reports** — programme status, weather, itemised "progress this week" and "plan for next week" lists (each line has an optional plot number tag plus the item text — add/edit/delete each line), H&S notes, issues/risks, commercial items raised that week (see below), a general "other comments" field, and photos. Every report has a clean printable view.
