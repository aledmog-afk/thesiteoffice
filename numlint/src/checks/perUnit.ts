...
import { singular } from '../extract/quantity.js';
...
export const perUnit: Rule = {
  ...
  run(ctx: RuleContext) {
    ...
    for (let i = 2; i < qs.length; i++) {
      ...
      if (avg.attributive) continue;
      // the count is the nearest plain number before the average; the total is the
      // nearest figure before that which is measured in the same thing as the average
      let count: Quantity | undefined;
      let total: Quantity | undefined;
      for (let j = i - 1; j >= 0; j--) {
        const cand = qs[j]!;
        if (cand.sentence !== avg.sentence) break;
        if (!count) {
          if (cand.kind !== 'plain' || cand.value <= 0 || cand.ordinal || cand.attributive || looksLikeYear(cand)) continue;
          count = cand;
          continue;
        }
        if (sameKind(cand, avg) && !cand.attributive) { total = cand; break; }
      }
      if (!count || !total) continue;
      const bridge = ctx.text.slice(total.span.end, count.span.start);
      if (!SPREAD.test(bridge) || bridge.length > 90) continue;
    ...
    // "the £4.2m scheme covers 200 residents, or £30,000 per resident" — the
    // per-unit noun names the count, so the two can be linked across the clause
    for (let i = 2; i < qs.length; i++) {
      const avg = qs[i]!;
      if (avg.kind === 'year' || avg.ordinal || avg.attributive) continue;
      const trail = ctx.text.slice(avg.span.end, avg.span.end + 40);
      const perNoun = /^[\s\u00a0]*(?:per|for\s+(?:each|every)|a|each)[\s\u00a0]+([a-z][\w-]*)/i.exec(trail);
      if (!perNoun) continue;
      const lead = ctx.text.slice(Math.max(0, avg.span.start - 40), avg.span.start).replace(/[$€£¥₹\s]+$/, ' ');
      if (!AVERAGE_LEAD.test(lead) && !/\b(?:works?\s+out\s+at|amounts?\s+to|equates?\s+to|equivalent\s+to|comes?\s+(?:out\s+)?to)\s*$/i.test(lead)) continue;
      const target = singular(perNoun[1]!.toLowerCase());
      ...
      for (let j = i - 1; j >= 0; j--) {
        const cand = qs[j]!;
        if (cand.sentence !== avg.sentence) break;
        if (!count && cand.kind === 'plain' && cand.noun === target && cand.value > 0 && !cand.attributive) { count = cand; continue; }
        if (!total && cand !== count && sameKind(cand, avg) && !cand.attributive) { total = cand; continue; }
        if (count && total) break;
      }
      if (!count || !total || total === count) continue;
      const expected = div(toInterval(total, ctx.options.slack), toInterval(count, ctx.options.slack));
      if (!expected) continue;
      const stated = toInterval(avg, ctx.options.slack);
      if (overlaps(stated, expected)) continue;
      ...
      ctx.report({
        rule: 'per-unit',
        severity: severityFor(stated, expected),
        confidence: confidenceFor(0.86, stated, expected),
        message: `${total.span.text} over ${count.span.text} ${perNoun[1]}s is ${fmtQuantityValue(point, { currency: avg.currency, unit: avg.unit?.surface })} each, not ${avg.span.text}.`,
        stated: avg.span.text,
        expected: fmtQuantityValue(point, { currency: avg.currency, unit: avg.unit?.surface }),
        workings: `${fmt(total.value)} ÷ ${fmt(count.value)} = ${fmt(point)}`,
        span: avg.span,
